/* This code is to use several functions to implement the floor model. Every function
has its own duty and play their own role in this big code  */
#include "floorplan.h"
...
int is_leaf_node(node_t* ptr) {
  ...
  if(ptr->left == NULL){
    return 1; /*if left child is NULL, it is leaf node*/
  }
  return 0;
}
...
int is_internal_node(node_t* ptr) {
  ...
  if((ptr->left != NULL) && (ptr->right != NULL)){
      return 1; /*if neither left child nor right child is NULL, it is internal node*/
  }
  return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  if(b->parent == a)
    return 1;
    if(b == a)
    return 1; /* if b and a are the same, a is parent of b, then it's true*/
  else if(b->parent == NULL)
    return 0; /* if b is root, it can't be a subtree*/
  else
    return is_in_subtree(a, b->parent); /*recursively check*/
}
...
if(ptr->module != NULL && ptr->cutline == UNDEFINED_CUTLINE){
  int t=ptr->module->w;
  ptr->module->w=ptr->module->h;
  ptr->module->h=t; /*swap width and height*/
  }
}
...
if(ptr->cutline == V)
    ...
    ptr->cutline=V; /*swap vertical and horizontal*/
  // TODO:
  return;
}
...
module_t* t=a->module;
a->module=b->module;
b->module=t; /*swap a and b, using temporary module*/
  // TODO:
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  node_t* temp;
if(a->parent->left == a)
  a_flag_left=1; /*use a flag to indicate which child of parent it is*/
if(a->parent->right == a)
a_flag_left=0;
...
if(b->parent->left == b)
  ...
  if(a_flag_left == 1 && b_flag_left == 1){ /*a is left child of parent of a, while b is left child of parent of b*/
    a->parent->left=b;
    b->parent->left=a;
    temp=a->parent;
    a->parent=b->parent;
  ...
  if(a_flag_left == 0 && b_flag_left == 1){ /* same method*/
    a->parent->right=b;
    b->parent->left=a;
    temp=a->parent;
    a->parent=b->parent;
  ...
  if(a_flag_left == 1 && b_flag_left == 0){ /* same method*/
    a->parent->left=b;
    b->parent->right=a;
    temp=a->parent;
    a->parent=b->parent;
  ...
  if(a_flag_left == 0 && b_flag_left == 0){ /* same method*/
    a->parent->right=b;
    b->parent->right=a;
    temp=a->parent;
    a->parent=b->parent;
...
postfix_traversal(ptr->right, nth, expression); /*using recursion to achieve post order*/

  expression[*nth].module=ptr->module;
  expression[*nth].cutline=ptr->cutline; /*print the data to the expression*/
(*nth)++; /*increment *nth*/
return;
...
if(n == num_modules-1){
  ...
  return ptr; /*base case, deal with parent, right child, left child, cutline and module*/
}

ptr->parent = par;
ptr->cutline=V;
ptr->module=NULL; /*don't need to consider left child and right child*/
...
ptr->right->left = NULL;
ptr->right->right = NULL; /*deal with stuff of right child*/


ptr->left=init_slicing_tree(ptr,n+1); /*for left child, recursively deal with it*/
return ptr;
